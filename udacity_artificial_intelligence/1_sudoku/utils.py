...
# own solution
def extract_units(unitlist, _boxes):
    units = {}

    for unit in unitlist:
        for box in unit:
            unit_members = [b for b in unit if b != box]

            units[box] = units[box] + \
                unit_members if box in units else unit_members
...
# own solution
def extract_peers(units, _boxes):
    peers = {}

    for unit in units:
        peers[unit] = list(set(units[unit]))
...
# own solution
# def grid2values(grid: str) -> dict:
#     values = {}

#     assert(len(grid) == len(boxes)), "Incorrect grid string length"

# #     values = dict(zip(boxes, grid)) # version 1
#     for b, g in zip(boxes, grid): # version 2 with all possible values
#         values.update ({b: "1234556789" if g == '.' else g })
